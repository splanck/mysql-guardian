...
void checkFailure(struct myserver *svr, struct mydatabase *db, struct mytable *tbl, checkType_t chk,
	...
	char error_email[250] = "";
	char error_subject[250] = "[MYSQLGD] - ";

	if(chk == c_serverOnline) {
		strcat(error_subject, svr->hostname);
		strcat(error_subject, " offline");
		...
		strcat(error_subject, svr->hostname);
		strcat(error_subject, " database inaccessible");
		...
		strcat(error_subject, svr->hostname);
		strcat(error_subject, " database offline");
		...
		strcat(error_subject, svr->hostname);
		strcat(error_subject, " integrity check failed");
		...
		strcat(error_subject, svr->hostname);
		strcat(error_subject, " database backup failed");
	...
	strcpy(error_email, error_msg);
	strcat(error_email, "\n\nEmail auto-generated by MySQL Guardian v");
	strcat(error_email, VERSION);
	strcat(error_email, "\n");

	sendEmail(error_email, error_subject);
